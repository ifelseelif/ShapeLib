...
(
    Id INT PRIMARY KEY,
    Name NVARCHAR(20) NOT NULL,
    CategoryIds varchar(max) default '' NOT NULL 
)
...
INSERT INTO Products VALUES (4, N'Орех', Default)
INSERT INTO Products VALUES (5, N'Груша Морская', '3,4')
...
SELECT Products.Name as 'Имя продукта', C.Name as 'Категория'
    FROM Products
LEFT JOIN Category C on C.Id IN (SELECT convert(int, value) FROM string_split(Products.CategoryIds, ','))